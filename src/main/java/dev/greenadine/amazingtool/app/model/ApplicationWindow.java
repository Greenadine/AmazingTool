...
import dev.greenadine.amazingtool.exception.InvalidWindowScreenException;
import dev.greenadine.amazingtool.utils.AnnotationUtils;
...
public abstract class ApplicationWindow extends JFrame {
     ...
     */
    protected void registerScreen(final String identifier, final ApplicationScreen applicationScreen) {
     ...
     */
    public void openScreen(final String screenName) {
     ...
     * @throws InvalidWindowScreenException If no {@code ApplicationScreen} with the given unique identifier exists.
     */
    public void openScreen(final String screenName, boolean hideWindow) {
        if (!applicationScreens.containsKey(screenName)) {
            throw new InvalidWindowScreenException(this, screenName);
        }
     ...
     */
    public void registerDialog(final ApplicationDialog applicationDialog) {
     ...
     */
    public ApplicationDialog showDialog(final String dialogName) {
     ...
     * @param dialogName The unique identifier of the {@code ApplicationDialog}.
     */
    public void hideDialog(final String dialogName) {
        if (!applicationDialogs.containsKey(dialogName)) {
            throw new IllegalArgumentException(String.format("Invalid dialog name '%s'.", dialogName));
        }
     ...
     * @param applicationDialog The {@code ApplicationDialog}.
     */
    public void hideDialog(final ApplicationDialog applicationDialog) {
        if (!AnnotationUtils.hasAnnotation(applicationDialog.getClass(), Identifier.class)) {
            throw new IllegalStateException("Dialog not annotated with identifier.");
        }
